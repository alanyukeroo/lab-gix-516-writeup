...
class BrainNode(Node):
    ...
    def __init__(self):
        ...
        # Launch aruco_markers node otomatis (one-command run)
            ...
            for x in range(1, width - 1, step):
                ...
                if data[idx] == 0:
                    ...
                    if -1 in neighbors:
                        ...
                        if dist > 0.5:
                            is_blacklisted = any(
                                math.sqrt((target_x - bx)**2 + (target_y - by)**2) < 0.2
                            ...
                            if not is_blacklisted:
                                ...
                                self._last_sent_goal = (target_x, target_y)
                                self.get_logger().info(f"Frontier baru: {dist:.2f}m")
                                self.send_nav_goal(frontier_pose, RobotState.EXPLORE)
                                return

        self.get_logger().info("Tidak ada area baru.", throttle_duration_sec=2.0)
    ...
    def save_aruco_in_map_frame(self):
        ...
        try:
            ...
            transform = self.tf_buffer.lookup_transform(
                'map',
                self.aruco_camera_pose.header.frame_id,  # oakd_rgb_camera_optical_frame
                rclpy.time.Time(),
    ...
    def nav_result_callback(self, future):
        ...
        if status == GoalStatus.STATUS_SUCCEEDED:
            ...
            self.get_logger().warn(f"Nav2 GAGAL (status={status}), blacklist & retry")
            if self._last_sent_goal and self._nav_next_state == RobotState.EXPLORE:
                self.blacklisted_frontiers.append(self._last_sent_goal)
            # Tetap di state yang sama untuk retry
        self.action_in_progress = False
...
def main(args=None):
    ...
    try:
        ...
        # Matikan aruco_markers process
        if hasattr(node, '_aruco_proc') and node._aruco_proc.poll() is None:
            node._aruco_proc.terminate()
            node.get_logger().info("aruco_markers node dihentikan.")
        node.destroy_node()
        rclpy.shutdown()
